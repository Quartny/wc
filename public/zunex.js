(function (window, document) {
  ...
  var chevronIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m12 17.59l-4.29-4.3l-1.42 1.42l5.71 5.7l5.71-5.7l-1.42-1.42z"/></svg>';
  ...
  function mount(options) {
    ...
    function connect(uuid) {
      var provider = providers.get(uuid) || window.Zunex.inBuildProvider;
      var error = modalRoot.querySelector('[data-error]');
      if (!provider) { error.textContent = uuid === inBuild.uuid ? 'Load the inBuild provider before connecting.' : 'Open the wallet extension and try again.'; return; }
      provider.request({ method: 'eth_requestAccounts' }).then(function (accounts) { account = { address: accounts[0] }; closeModal(); renderAccount(); }).catch(function (reason) { error.textContent = reason.message || 'Could not connect to this wallet.'; });
    }
    function openModal() {
      var list = getWallets();
      modalRoot.innerHTML = '<div class="zunex-backdrop" data-backdrop><section class="zunex-modal" role="dialog" aria-modal="true"><div class="zunex-modal-head"><h2>Connect a wallet</h2><button class="zunex-close" data-close aria-label="Close">×</button></div><p class="zunex-subtitle">Choose an installed wallet to continue.</p><div class="zunex-wallet-list">' + (list.length ? list.map(function (wallet) { return '<button class="zunex-wallet" data-wallet="' + escapeHtml(wallet.uuid) + '">' + walletIcon(wallet) + '<span class="zunex-wallet-info"><strong>' + escapeHtml(wallet.name) + '</strong><small>' + escapeHtml(wallet.rdns || 'Browser extension') + '</small></span><span class="zunex-arrow">→</span></button>'; }).join('') : '<div class="zunex-empty"><strong>No wallet extensions found</strong><small>Install a compatible wallet extension, then reopen this panel.</small></div>') + '</div><p class="zunex-error" data-error></p></section></div>';
